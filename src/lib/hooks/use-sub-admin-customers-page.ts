...
} from '../api/endpoints/subAdminApi';

export function useSubAdminCustomersPage() {
  ...
  } = useFetchSubAdminCustomersQuery(undefined, {
  ...
  const [createCustomer, { isLoading: isCreating }] = useCreateSubAdminCustomerMutation();
  const [updateCustomer, { isLoading: isUpdating }] = useUpdateSubAdminCustomerMutation();
  const [deleteCustomer] = useDeleteSubAdminCustomerMutation();
